...
function attackShip(atkShip = Ship,dShip,weap = Number,hit = Number,undo = false) {
	let atkdata = atkShip.attack(weap,undo);
	let aWeap = atkdata[0];
	let defShip;
	if (!(aWeap.Type === "Deploying" || aWeap.Type === "Destruct")) defShip = dShip;
	let defdata;
	...
	if (hit === -1) {
		if (aWeap.Type === "Deploying") {
			hit = atkShip.rot;
			dShip = copyArray(atkShip.position);
			if (atkShip.isInLocation(cursorLoc)) dShip = copyArray(cursorLoc);
			attackList.push([atkShip.number,weap,[0],dShip,hit]);
		} else if (aWeap.Type === "Healing") {
			let dist = calculateDist(atkShip.location,defShip.location);
			hit = 2*(dist <= 1);
			attackList[attackList.length] = [atkShip.number,weap,defShip.player,defShip.number,hit];
		} else if (aWeap.Type === "Destruct") {
			let DataList = [[],[],[]]; //Player, ShipNum, hit
			let [sx,sy] = cursorLoc[cursorLoc.length -1];
			let cSector = cursorLoc.slice(0,-1);
			let hitSList = [[],[],[]];
			let defShips = [];
			let wRan = aWeap.Wran;
			let wHit = aWeap.Whit;
			let acc = atkdata[1]; // + factionTraits[atkShip.player.Faction][2]*(factionTraits[atkShip.player.Faction][10]) + factionTraits[atkShip.player.Faction][2];
			...
			attackList.push([atkShip.number,weap,DataList[0],DataList[1],DataList[2]]);
			defShip = defShips;
			hit = DataList[2];
		} else if (aWeap.type === "Wait") {
			attackList[attackList.length] = [atkShip.number,weap,dShip.player,dShip.number,0];
		} else {
			let wHit = aWeap.Whit;
			...
			attackList.push([atkShip.number,weap,defShip.player,defShip.number,hit]);
		}
	} else {
		if (aWeap.type === "Destruct") {
				for (let v = 0; v < dShip[0].length; v++) {
					defShip.push(dShip)
				}
		} else if (aWeap.Type === "Deploying") dShip = dShip[1];
	}
	if (weap === -1) {
		return;
	}
	...
	switch (aWeap.Type) {
		...
		case "Deploying":
			if (undo) {
				...
				let playerNum = atkShip.player
				let player = Players[playerNum];
				let nShip = new Ship(playerNum,player.ShipList.length,atkShip.faction,aWeap.dType,dShip,hit);
		...
		case "Destruct":
			...
			defShip.forEach((dShip,i) => {
				def = dShip.getDefenses()[0];
				dist = calculateDist(atkShip.location,dShip.location);
				damage = wAtk - def + wRAtk*(dist + (wRan - dist)*(wRAtk >= 0 && dist >= wRan));
				damage *= 100; // + Math.round(factionTraits[atkShip.player.Faction][0]*(1-(atkShip.oldHP/atkShip.maxHP)))*(factionTraits[atkShip.player.Faction][9]) + factionTraits[atkShip.player.Faction][0]*(!factionTraits[atkShip.player.Faction][9]) + factionTraits[atkShip.player.Faction][1];
				damage = Math.round(damage/100);
				damage = damage*(damage > 0);
				dShip.defend(damage*(Number(hit[i]) === 2),aWeap.Type,undo);
				if (!undo) {
					if (Number(hit[i]) === 2) {
						str += `${Players[atkShip.player].Name}'s ${atkShip.Name} hits ${Players[dShip.player].Name}'s ${dShip.Name} for ${damage} HP.\n`;
					} else {
						str += `${Players[atkShip.player].Name}'s ${atkShip.Name} misses ${Players[dShip.player].Name}'s ${dShip.Name}.\n`;
					}
				}
			});
		break;
...
function endTurn () {
	if ((stage === 0 || stage === 1 || stage === 2) && Players.length > 1) {
		...
		buttons.style.visibility = "hidden";
		input.style.visibility = "visible";
		Players[0].AP = Players[0].ShipList.length;
		updateInputBox(); 
		updateDataString();
...
function updateImpulse () {
	let APCount = Infinity;
	let player = -1;
	impulseAP = 2*Math.ceil(APCount);
	impulseCount = Math.ceil(APCount/impulseAP) + 1;

	Players.forEach((val,index) => {
		val.updateAP();
		let tImpulseAP = 2*Math.ceil(val.AP/16);
		let tImpulseCount = Math.ceil(val.AP/tImpulseAP) + 1;

		if (tImpulseCount < impulseCount || !impulseCount) {
			APCount = val.AP;
			player = index;
		}
